...
RUN mkdir -p /ea-bin
RUN go mod download 
RUN go build -a -o /ea-bin/easy-audit /github.com/jnnkrdb/easy-audit/cmd/server/main.go
RUN go build -a -o /ea-bin/eactl /github.com/jnnkrdb/easy-audit/cmd/cli/main.go
# ---------------------------------------------------------------------------------------------- Final Alpine
FROM alpine:3.24.1
